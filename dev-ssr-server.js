...
app.use('*', async (req, res, next) => {
  try {
    ...
    // Let Vite handle static assets and dev requests
    if (url.includes('/@vite') || url.includes('/src/') || url.includes('/node_modules/') || url.includes('.js') || url.includes('.css') || url.includes('.svg') || url.includes('.ico') || url.includes('.png') || url.includes('.jpg') || url.includes('.jpeg') || url.includes('.webp')) {
      return next()
    }
    
    // Use Vite's transformIndexHtml to get the proper HTML with all assets
    const html = await vite.transformIndexHtml(url, await generateHTML(url, '<div id="root"></div>'))
...
createVite().then(() => {
  app.listen(3000, () => {
    console.log('🚀 SSR server with metadata running at http://localhost:3000')
    console.log('✅ Metadata works + Vite dev server for UI!')
  })
})
